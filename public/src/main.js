...
jQuery(document).ready(function(){
   ...
   opacitySlider.onDrag = function(s){
      ...
      myApp.startLogging();
      var rgbaColor = "rgba(" + myApp.PaintingAttribute.ForeColor.r + "," + myApp.PaintingAttribute.ForeColor.g + "," + myApp.PaintingAttribute.ForeColor.b + "," + myApp.PaintingAttribute.ForeColor.a + ")";
   ...
   * please adjust binders to the appropriate menu elements
   ...
   */
   ...
   $("#tb_tool .clear_image").bind("click",function(){myApp.clearImage();});   
   $("#tb_tool .insert_image").bind("click",function(){myApp.insertImageDialog();});
   ...
   //
   ////////////////////////////////////////////////////////
   var rulerSizeHandler = 
            function(){
               var w = myApp.toPixel(parseFloat($("#ruler_canvas_width").val()),myApp.getUnit());
               var h = myApp.toPixel(parseFloat($("#ruler_canvas_height").val()),myApp.getUnit());
               if ( w !== myApp.width || h !== myApp.height ){
                  
                  myApp.sizeTo(w,h);
                  
                  myApp.canvas_history.add("resize", "resize|" + w + "," + h);
               }
            };
      
   $("#ruler_canvas_width,#ruler_canvas_height")
      .bind("blur", rulerSizeHandler)
   ...
   /**
    TODO: this operation should be done from menu,
    please attach the correct menu element:
   ...
   */
   ...
   function initGUI(){
      ...
      $( document ).bind("keyup", function(event) {
         ...
         var isNotEditing = "INPUT" != document.activeElement.tagName.toUpperCase() 
                           && "TEXTAREA" != document.activeElement.tagName.toUpperCase() 
                           && "SELECT" != document.activeElement.tagName.toUpperCase();
      

         if( isNotEditing && event.keyCode == 46) {

               myApp.deleteActiveObject();
      ...
      $( document ).bind("keydown", function(event) {
         ...
         if ( isNotEditing && (event.keyCode == 34 || event.keyCode  == 32) ) {
               return false;
         }                           
...
function fixFirefoxFormSelection() {  
   $("TEXTAREA").bind("focus", enableSelection);
   $("INPUT").bind("focus", enableSelection);
   
   $("TEXTAREA").bind("blur", disableSelection);
   $("INPUT").bind("blur", disableSelection);
