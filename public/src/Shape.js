...
G_Shape.prototype.toString = function(delimiter){
   ...
   var serializedObj = s1 + delimiter + s.join(",");
...
function G_Line(){this.parent.constructor.call(this);this.type = "Line";this.closeShape = false;}
...
function G_Rectangle(){this.parent.constructor.call(this);this.type = "Rectangle";this.closeShape = true;}
...
G_Shape.getShape = function(sData,delimiter){
   ...
   switch(type){
   case "Line":
      targetObject = new G_Line().deserialize(sData,delimiter);
      break;
   case "Polyline":
      targetObject = new G_Polyline().deserialize(sData,delimiter);      
      break;
   case "Pentagon":
   case "Diamond":
   case "4-point star":
   ...
   case "Up arrow":
   case "Down arrow":
      targetObject = new G_Polygon().deserialize(sData,delimiter);
      break;
   case "Bezier curve":
      targetObject = new G_Curve().deserialize(sData,delimiter);
      break;
   case "Oval":
      targetObject = new G_Oval().deserialize(sData,delimiter);
      break;
   case "Cardinal spline":
      targetObject = new G_CardinalSpline(false).deserialize(sData,delimiter);
      break;
   case "Closed cardinal spline":
      targetObject = new G_CardinalSpline(true).deserialize(sData,delimiter);
      break;
   case "Rounded rectangular callout":
      targetObject = new RoundedRectangularCallout().deserialize(sData,delimiter);
      break;
   case "Heart":
      targetObject = new G_Heart().deserialize(sData,delimiter);
      break;
   case "Rectangle":
      targetObject = new G_Rectangle().deserialize(sData,delimiter);
      break;
   case "Rounded rectangle":
      targetObject = new G_RoundedRectangle().deserialize(sData,delimiter);
      break;
   case "openimage":
      targetObject = new G_InsertedImage().deserialize(sData,delimiter);
      break;      
   }
...
EXTEND(G_InsertedImage,G_Shape);

function G_InsertedImage(img,ctx,canvasImage){   
   this.parent.constructor.call(this);
   this.closeShape = true;
   
   this.originalCan = canvasImage;
   this.img = img;
   this.myCtx = ctx;
}

G_InsertedImage.prototype.init = function() {
   this.type = "shape";
   this.nodes.push(new G_Point(0,0));
   this.nodes.push(new G_Point(parseInt(this.img.width),0));
   this.nodes.push(new G_Point(0,parseInt(this.img.height)));
   this.nodes.push(new G_Point(parseInt(this.img.width),parseInt(this.img.height)));
   
   this.minX = this.nodes[0].x;
   this.maxX = this.nodes[3].x;
   this.minY = this.nodes[0].y;
   this.maxY = this.nodes[3].y;

   this.draw();
}

G_InsertedImage.prototype.draw = function() {   
   if(!("img" in this)) {
      this.img = this.getImageElement();
   }
   this.myCtx.putImageData(this.originalCan,0,0);
   
   this.myCtx.drawImage(this.img,
                        this.minX,
                        this.minY,
                        this.width(),
                        this.height()
...
G_InsertedImage.prototype.deserialize = function(sData,delimiter) {
   delimiter = delimiter || "|";
   ...
   this.nodes.push( new G_Point( this.minX, this.minY ) );
   this.nodes.push( new G_Point( this.maxX, this.minY ) );
   this.nodes.push( new G_Point( this.minX, this.maxY ) );
   this.nodes.push( new G_Point( this.maxX, this.maxY ) );
   
   return this;
};

G_InsertedImage.prototype.toString = function() {
   this.myCtx = document.getElementById("ipaint_frame_canvas").getContext("2d");
   return "openimage" 
            + "|" 
            + (this.minX)
            + "|"
...
G_InsertedImage.prototype.asDataURL = function() {
   return this.getImageElement().toDataURL();
};

G_InsertedImage.prototype.getImageElement = function() {
   var tmpCan  = document.createElement("canvas");
   tmpCan.width   = this.maxX-this.minX;
   tmpCan.height  = this.maxY-this.minY;
   var imData = this.myCtx.getImageData(this.minX,this.minY,this.maxX-this.minX,this.maxY-this.minY);
   tmpCan.getContext("2d").putImageData(imData,0,0);
   return tmpCan;
};

G_InsertedImage.prototype.rotate = function(degree){
   var imData = this.myCtx.getImageData(this.minX,this.minY,this.maxX-this.minX,this.maxY-this.minY);
   var processedImData = new ImageProcessor( imData ).rotate(degree);

   this.parent.rotate.call(this,degree,false);

   var tmpCan  = document.createElement("canvas");
   
   tmpCan.width   = processedImData.width;
   tmpCan.height  = processedImData.height;
   tmpCan.getContext("2d").putImageData(processedImData,0,0);
...
EXTEND(G_Hyperlink,G_Shape);

G_Hyperlink.nextId = 0;

/**
* @param link URL for the link
*/
function G_Hyperlink() {
   this.parent.constructor.call(this);
   this.type = "hyperlink";
   this.delimiter = ";";
}

G_Hyperlink.prototype.init = function(canvas_event,id) {
   
   this.initId(id);
   
   this.canvas_event = canvas_event;
   this.link = "";
   this.type = "hyperlink";
   this.minX = 0;
   this.minY = 0;
   ...
   this.setNodesFromMins();
   this.prepareDiv();
};

G_Hyperlink.prototype.initId = function(id) {
   if (!id){
      id = ++G_Hyperlink.nextId;
   }
   else if ( G_Hyperlink.nextId < id ){
      id = parseInt(id);
      G_Hyperlink.nextId = id + 1;
   }
   this._id = id;
};

G_Hyperlink.prototype.prepareDiv = function() {
   this.isDivAdded = false;
   this.div = document.createElement("div");
   this.div.id = this._id;
   
   var canvas_event = this.canvas_event;
   var _this = this;
   $(this.div).bind("dblclick", function(ev) {
      window.open(_this.link);
   });
   
   $(this.div).bind("mousemove",function(ev) {
      $(canvas_event).trigger(ev);
      return;
   });
   
   $(this.div).bind("click",function(ev) {
      $(canvas_event).trigger(ev);
   });
   ...
   $(this.div).bind("mouseup",function(ev) {
      $(canvas_event).trigger(ev);
   });   
};

G_Hyperlink.prototype.setNodesFromMins = function() {
   this.nodes = [];
   this.nodes.push( new G_Point( this.minX, this.minY ) );
   this.nodes.push( new G_Point( this.maxX, this.minY ) );
   this.nodes.push( new G_Point( this.minX, this.maxY ) );
...
G_Hyperlink.prototype.draw = function(minX,minY,maxX,maxY) {

   if(this.div == null) {
      this.prepareDiv();
   }
   
   if( minX && maxY && maxX && maxY) {
   ...
   if( (0 == this.minX && 0 == this.maxX)
      || (0 == this.minY && 0 == this.maxY)) {
      return;
   }
   
   if( !this.isDivAdded ) {
      $("#inner_container").append(this.div);
      $(this.div).addClass("hyperlink_container hyperlink_container_active");
      this.isDivAdded = true;
   }
   
   this.div.style.top = (this.minY + 5) + "px";
   this.div.style.left = (this.minX + 5) + "px";
   this.div.style.width = (this.maxX - this.minX) + "px";
   this.div.style.height = (this.maxY - this.minY) + "px";
   
   $(this.div).text(this.link);
};

G_Hyperlink.prototype.deserialize = function(str,canvasEvent) {
   ...
   this.canvas_event = canvasEvent;   

   this.minX = parseInt( ar[ 0 ] );
   this.minY = parseInt( ar[ 1 ] );
   this.maxX = parseInt( ar[ 2 ] );
   this.maxY = parseInt( ar[ 3 ] );
   this.link = ar[ 4 ] ;
...
G_Hyperlink.prototype.serialize = function() {
   var delimiter = this.delimiter;
   var ar = [];
   ar.push( this.minX );
   ar.push( this.minY );
...
function G_TextShape(iPaintApp) {
   this.parent.constructor.call(this); 
   this.paintApp = iPaintApp;
};

G_TextShape.prototype.init = function() {
...
G_TextShape.prototype.draw = function() {
   
   var el = $("#text_input")
      .css("visibility", "visible")
      .css("left",     (this.minX + 4) + "px")
      .css("top",      (this.minY + 4) + "px")
   ...
   this.setNodesFromMins();
   this.paintApp.setupFont();
};

G_TextShape.prototype.setNodesFromMins = function() {
   this.nodes = [];
   this.nodes.push( new G_Point( this.minX, this.minY ) );
   this.nodes.push( new G_Point( this.maxX, this.minY ) );
   this.nodes.push( new G_Point( this.minX, this.maxY ) );
