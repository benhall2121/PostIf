...
//                             "\" class=\"drag_box canvas_drag_down\" style=\"width:" + this.RulerSize + "px;height:10px;visibility:hidden;\"></div>")
      ...
      .append(
         $("<div></div>").attr("id","inner_container").addClass("scroller").attr("tabindex","1")
         .css({
         ...
         .append("<canvas style=\"padding-bottom:12px\" id=\"" + this.getID("canvas_temp")            + "\" width=\"" + this.width + "\" height=\"" + this.height + "\" class=\"canvas_temp\"></canvas>")         
...
//         .append("<div    id=\"" + _this.getID("canvas_drag_right_down") + "\" class=\"drag_box canvas_drag_right_down\"></div>")
         .append("<textarea id=\"text_input\"></textarea>")
...
iPaintApp.prototype.setupFont = function(){
   ...
   $("#text_input").css({
      ...
      fontWeight:this.PaintingAttribute.font.weight,
      lineHeight: this.PaintingAttribute.font.size
   });
   ...
   if(this.PaintingAttribute.font.style == "italic") {
      $(".font_italic").addClass("selected");
   }
   else {
     $(".font_italic").removeClass("selected");
   }
   
   if(this.PaintingAttribute.font.weight == "bold") {
      $(".font_bold").addClass("selected");
   }
   else {
     $(".font_bold").removeClass("selected");
   }
   
   $(".font_family").val(this.PaintingAttribute.font.name);
   $(".font_size").val(this.PaintingAttribute.font.size);
};
...
iPaintApp.prototype.sizeTo = function(w,h){
   ...
   var canvasImageData = this.context.getImageData(0,0,this.width,this.height);
   var canvasImage = this.createCanvas( canvasImageData );
   ...
   this.context.drawImage(canvasImage, 0, 0);
...
iPaintApp.prototype.drawImageShape = function(x,y,srcUrl,callback){
   var _paintApp = this;
   var oImage = new Image;

   oImage.onload = function() {     
      var canvasImage = _paintApp.context.getImageData(0,0,_paintApp.width, _paintApp.height);
      var shape = new G_InsertedImage(this,_paintApp.context_draft,canvasImage);
      shape.init();     
      shape.move(parseInt(x),parseInt(y)).draw();
      callback();
   };
...
iPaintApp.prototype.insertImageBySrc = function(url){
   var s  = document.domain.toLowerCase();
   var bIsRemote = (url.toLowerCase().indexOf(s) === -1);
   var bIsDataURL = (url.toLowerCase().indexOf("data:image/") !== -1);
   var _this = this;
   if ( bIsRemote && !bIsDataURL ){
      $.getImageData({
         url:url,
         success:function(serverImage){
            if(null != _this.trackableObject ) {
               _this.trackableObject.done();
            }
            _this.trackableObject = new InsertedImageObject();
            _this.trackableObject.init(_this,serverImage);
         },
      ...
      img.onload = function(){
         _this.trackableObject = new InsertedImageObject();
         _this.trackableObject.init(_this,this);
      };
      img.src = url;
   }
};
...
iPaintApp.prototype.getAction = function(){
...
iPaintApp.prototype.setAction = function(sType){
   ...
   case "cut":
      ...
      this.trackableObject = null;
      $(".tool_cut").removeClass("selected");
      if (typeof(disableIcon) === "function" ){
         disableIcon("tool_crop","tool_resize","tool_rotate","tool_copy", "tool_cut");
      }
      return;
   case "copy":
      this.copyClipboard();
   ...
   case "delete_element":
      ...
      $(".tool_delete").removeClass("selected");
      break;
   case "bucket_solid":          
   ...
   case "zoom_glass"                  : this.trackableObject = new ZoomGlassObject(this.trackableObject);break;
   
   case "add_hyperlink": 
      this.trackableObject = new AddHyperlinkObject(this);
      if (typeof(disableIcon) === "function" ){
         disableIcon("tool_resize","tool_rotate","tool_cut", "tool_copy");
      }
      break;
   }
      ...
      if ( this.trackableObject.ObjectType !== "brush" ){
         ...
         if ( !this.bucketObject ){
            ...
            if ( this.trackableObject.type === "shape" ){
               ...
               if(null != this.clipboard && typeof(enableIcon) === "function") {
                  enableIcon("tool_paste");
               }
            }else if ( this.trackableObject.type === "selection" ){
               this.trackableObject.onCompleted = 
                  function(obj){
                     if (typeof(enableIcon) === "function" ){
                        enableIcon("tool_crop","tool_resize","tool_rotate","tool_cut", "tool_copy");
                     }
                  };
               this.trackableObject.onDone = 
                  function(obj){
                     if (typeof(disableIcon) === "function" ){
                        disableIcon("tool_crop","tool_resize","tool_rotate","tool_cut", "tool_copy");
                     }
                  };
            }
         }
...
iPaintApp.prototype.restoreHistoryTo = function(toIndex){
   ...
   this.context_draft.clearRect(0,0,this.width,this.height);
   this.removePreviousHyperlinks();
   this.restoreHistoryRange(0,toIndex);
};

iPaintApp.prototype.restoreHistoryRange = function(start,nTotal){
   if (start >= nTotal){
      return;
   }
   ...
   var callback = function() {
      ...
      if(next < nTotal) {
         _this.restoreHistoryRange(next, nTotal);
      }
   };
...
iPaintApp.prototype.restoreStorage = function(s, callback){
   s   = s.split("@")[1];
   ...
   case "New":
      ...
      break;
   case "openimage":
      this.drawImageShape(arr[1],arr[2],arr[5],callback);
      break;
   case "foreground":
   ...
   case "solidfill":
      this.context_draft.drawImage(this.canvas,0,0);
      this.context.clearRect(0,0,this.width,this.height);
   ...
   case "Hyperlinks":
      this.getHyperlinksFromString(arr[1]);
      break;
   default:
      var ctx = {
...
iPaintApp.prototype.insertImageDialog = function(){
   var app = this;
...
iPaintApp.prototype.openRotateDialog = function(){
   var app = this;
   return new jsWindow(this.winMgr)
      .createDialog("rotate_image","Rotate Image ...",310,170)
               ...
               var oFormInitValue = 
                              {
                                 ...
                                 BtnCancel : function(jwin){
                                                dialog.cancelApply(app.trackableObject);
                                                jwin.close("Cancel"); 
                                             },
                                 BtnClose : function(jwin){
                                                dialog.commitRotation(app.trackableObject);
                                                jwin.close("Close"); 
                                             }
                              };
               jWin.setFormData(oFormInitValue);
...
*
*
* @param x X-coordinates related to the canvas itself
* @param y Y-coordinates related to the canvas itself
*/
iPaintApp.prototype.findHyperlinkByCoord = function(x,y) {
   var hyperlink = null;

   for(var i in this.hyperlinks) {
      var obj = this.hyperlinks[ i ];
      
      if ( this.isCoordInsideShape(x,y,obj) ){            
            hyperlink = obj;
      }
   }  
...
iPaintApp.prototype.isCoordInsideShape = function(x,y,obj) {
   return null != obj 
            && x >= obj.minX 
            && y >= obj.minY
            && x <= obj.maxX
...
iPaintApp.prototype.findHyperlinkById = function(id) {
   var hyperlink = null;
...
iPaintApp.prototype.getHyperlinksFromString = function(str) {
   this.removePreviousHyperlinks();
   var ar = str.split(",");
   this.hyperlinks = [];
   for( var i in ar) {
      var objString = ar[i];
      var oHyperlink = new G_Hyperlink();
      oHyperlink.deserialize(objString, this.event_canvas);
      oHyperlink.draw(oHyperlink.minX,oHyperlink.minY,oHyperlink.maxX,oHyperlink.maxY);
      this.hyperlinks.push(oHyperlink);
...
iPaintApp.prototype.removePreviousHyperlinks = function() {
   for (var i in this.hyperlinks) {
      var obj = this.hyperlinks[ i ];
      var el = $(obj.div);
      if(el) {
...
iPaintApp.prototype.getHyperlinksAsString = function() {
   var s = [];
   for(var i in this.hyperlinks) {
      var obj = this.hyperlinks[ i ];
      s.push(obj.serialize());
   }      
   var result = "Hyperlinks|" + s.join(",");

   return result;
};

iPaintApp.prototype.deleteActiveObject = function() {

   var isSelected = ("visible" == $("#focus_ring").css("visibility"));
   
   if ( null != this.trackableObject && isSelected ) {
      this.trackableObject.deleteShape();
   }
};

iPaintApp.prototype.addToClipboard = function(imageData,shape) {
   var canvas = document.createElement("canvas");
   
   if ( shape && shape.nodes.length > 1 ) {
      
      var wholeCanvas = document.createElement("canvas");
      wholeCanvas.width = imageData.width;
      wholeCanvas.width = imageData.width;
      wholeCanvas.getContext("2d").putImageData(imageData,0,0);
      ...
      context.moveTo(
         shape.nodes[0].x - shape.minX,
         shape.nodes[0].y - shape.minY
      );
         
      for ( var i in shape.nodes ) {
         var currentNode = shape.nodes[ i ];
         context.lineTo( currentNode.x - shape.minX,
                         currentNode.y - shape.minY
         );
      }
      
      context.save();
      context.clip();
      context.drawImage(wholeCanvas,0,0);
      context.restore();
   }
   else {
      canvas.width = imageData.width;
...
iPaintApp.prototype.copyClipboard = function() {
   if ( this.trackableObject ){
      this.trackableObject.copy();
   }

   $(".tool_copy").removeClass("selected");
   $(".tool_paste").removeClass("disabled");
};

iPaintApp.prototype.pasteClipboard = function() {
   this.disableHEM();
   
   if(null != this.trackableObject ) {
      this.trackableObject.done();
   }
   this.trackableObject = new InsertedImageObject();
   this.trackableObject.init(this, this.clipboard, "Paste");
   
   //this.context.drawImage(this.clipboard,0,0);
   
   $(".tool_paste").removeClass("selected");
};

/**
* HEM - hyperlinks editing mode
*/
iPaintApp.prototype.enableHEM = function() {
   $(".hyperlink_container").addClass("hyperlink_container_active");
   for(var i in this.hyperlinks) {
      var h = this.hyperlinks[i];
      $(h.div).text(h.link);
   }
};

iPaintApp.prototype.disableHEM = function() {
   $(".hyperlink_container").removeClass("hyperlink_container_active");
   $(".hyperlink_container").text("");
};

iPaintApp.prototype.drawText = function(string) {
   var arr = string.split( TextObject.historySeparator );
   
   TextObject.renderText(this.context, 
                         arr[0],
                         arr[1],
                         arr[2],
                         arr[3],
...
iPaintApp.prototype.rotateSelection = function(str) {
   var ar = str.split("~");
   var shapeStr = ar[0].replace(/Selection/g,"Polygon");
  
   var angle = parseInt(ar[1]);
   var shape = G_Shape.getShape(shapeStr,"#");
   
   var selection = new G_PolygonSelection();
   
   selection.nodes = shape.nodes;
   selection.minX = shape.minX;
   selection.minY = shape.minY;
   selection.maxX = shape.maxX;
   selection.maxY = shape.maxY;
   
   selection.select(this.canvas,true)
         .rotate(angle*1)
         .draw(
           {
