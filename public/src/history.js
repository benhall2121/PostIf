...
History.prototype.synchronizeHistory = function(){
   ...
   this.paintApp.restoreHistoryRange(0,nTotal);
};
...
History.prototype.updateItem = function(index,sLabel,data){
   var status = false;
   if ( null != window.localStorage.getItem("hist_" + index) ) {
      window.localStorage.removeItem("hist_" + index);   
      this.history[index] = {label:sLabel,data:data};
      window.localStorage.setItem("hist_" + index,sLabel + "@" + data);
      status = true;
   }   
   return status;
};
